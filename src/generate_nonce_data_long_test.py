...
CPU_NUM = min(1, multiprocessing.cpu_count())
NLP = None
BATCH_SIZE = 64
NONCE_WORD_BANK = {}

SENT_NUM = 0
SENT_NUM_WHERE_TOKEN_FAILED_TO_MATCH = 0
SENT_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH = 0
TOKENS_NUM_NOT_REPLACED = 0
CONTENT_WORD_NUM = 0
DATA_NUM_WHERE_TOKEN_FAILED_TO_MATCH = 0
DATA_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH = 0
...
def generate_nonce_sentence(doc, max_n: int) -> tuple[list[str], int]:
    ...
    """
    # get content words
    global SENT_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH
    global TOKENS_NUM_NOT_REPLACED
    global CONTENT_WORD_NUM
    global SENT_NUM_WHERE_TOKEN_FAILED_TO_MATCH
    content_words = [token for token in doc if is_content_word(token)]

    # get nonce words for each content word
    tokens_failed_to_match = 0
    ...
    for token in content_words:
        ...
        if not candidates:
            ...
            tokens_failed_to_match += 1
            # skip a sentence if over half contents failed to match nonce words
            if tokens_failed_to_match >= len(content_words) / 2:
                SENT_NUM_WHERE_TOKEN_FAILED_TO_MATCH += 1
                return [""] * max_n, -1

        if max_n > len(candidates):
            candidates = candidates * ((max_n + len(candidates)) // len(candidates))
    ...
    if tokens_failed_to_match > 0:
        SENT_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH += 1
        TOKENS_NUM_NOT_REPLACED += tokens_failed_to_match
        CONTENT_WORD_NUM += len(content_words)
    ...
    return nonce_sentences, tokens_failed_to_match 
...
# ================= Dataset Processing =================
def _safe_texts(texts, max_len):
    for t in texts:
        yield t[:max_len]

def _process_text(text, max_n) -> list[str]:
    global SENT_NUM
    global SENT_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH
    global DATA_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH
    global DATA_NUM_WHERE_TOKEN_FAILED_TO_MATCH 
    # split text to sents
    sents = simple_split_text(text)

    # generate nonce sents
    # shape of nonce: [#sent, max_n]
    nonce_sents = []
    # need the full pipeline for sentence segmentation
    docs = NLP.pipe(_safe_texts(sents, NLP.max_length), batch_size=64)
    failed_sents = 0
    data_failed = False
    for doc in docs:
        # generate max_n nonce snets for each doc
        SENT_NUM += 1
        _sents, _failed_tokens = generate_nonce_sentence(doc, max_n)
        nonce_sents.append(_sents)
        if _failed_tokens >= 0:
            failed_sents += 1
        if _failed_tokens < 0:
            data_failed = True
    if failed_sents > 0:
        DATA_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH += 1
    if data_failed:
        DATA_NUM_WHERE_TOKEN_FAILED_TO_MATCH += 1

    
    # shape of nonce_texts: [max_num, ]
    # concat nonce sentenses to long text
    nonce_texts = [" ".join(s) for s in zip(*nonce_sents)]
    
    return nonce_texts

def map_nonce_generation(examples, max_n: int):
    # shape: [#examples, max_N]
    nonce_texts = [_process_text(text, max_n) for text in _safe_texts(examples["text"], NLP.max_length)]

    texts = []
    nonce = []
    for i, _nonce_texts in enumerate(nonce_texts):
        for __nonce_text in _nonce_texts:
            if __nonce_text.strip():
                texts.append(examples["text"][i])
                nonce.append(__nonce_text.strip())
    return {
        "text": texts,
        "nonce": nonce
    }
...
def generate_nonce_for_dataset(
    ...
    out_path_word_bank = Path(out_path) / "nonce_word_bank.pkl"
    ...
    if nonce_word_bank_path and Path(nonce_word_bank_path).exists():
        ...
        with open(nonce_word_bank_path, "rb") as f:
            NONCE_WORD_BANK = pickle.load(f)
        NONCE_WORD_BANK = {k: list(set([tuple(t) for t in v])) for k, v in tqdm.tqdm(NONCE_WORD_BANK.items())}
        print("**done")
    ...
    process_fn = partial(map_nonce_generation, max_n=max_n)
    dataset = dataset.map(
        process_fn,
        num_proc=None,
...
if __name__ == "__main__":
    ...
    print("SENT_NUM_WHERE_TOKEN_FAILED_TO_MATCH:", SENT_NUM_WHERE_TOKEN_FAILED_TO_MATCH)
    print("SENT_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH:", SENT_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH)
    print("TOKENS_NUM_NOT_REPLACED:", TOKENS_NUM_NOT_REPLACED)
    print("CONTENT_WORD_NUM:", CONTENT_WORD_NUM)
    print("DATA_NUM_WHERE_TOKEN_FAILED_TO_MATCH:", DATA_NUM_WHERE_TOKEN_FAILED_TO_MATCH)
    print("DATA_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH:", DATA_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH)

    # 100
    # SENT_NUM: 2811
    # SENT_NUM_WHERE_TOKEN_FAILED_TO_MATCH: 38
    # SENT_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH: 1250
    # TOKENS_NUM_NOT_REPLACED: 2063
    # CONTENT_WORD_NUM: 16468
    # DATA_NUM_WHERE_TOKEN_FAILED_TO_MATCH: 29
    # DATA_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH: 94

    # 1000
    # SENT_NUM: 36917
    # SENT_NUM_WHERE_TOKEN_FAILED_TO_MATCH: 514
    # SENT_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH: 16643
    # TOKENS_NUM_NOT_REPLACED: 27678
    # CONTENT_WORD_NUM: 229868
    # DATA_NUM_WHERE_TOKEN_FAILED_TO_MATCH: 263
    # DATA_NUM_LESS_HALF_TOKENS_FAILED_TO_MATCH: 926