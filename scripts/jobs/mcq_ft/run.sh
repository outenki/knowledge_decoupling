    ...
    case $1 in
        ...
        -f|--ft-data)
            if [[ -n "$2" && "$2" != -* ]]; then
                ...
                echo "err: -f | --ft-data need a value"
                exit 1
            fi
            ;;
        -t|--evaluate-data)
...
[[ -z "$FT_DATA" ]] && missing_args+=("--ft-data")
...
run_train() {
    ...
    uv run python "$SCRIPT_PATH/train/mcq_finetune.py" \
        --speedup \
        -cn "$CONFIG_NAME" \
        -im "$in_model" \
        -dp "$data_path" \
...
for ft_split in train
do
    echo
    echo
    echo ">>>>>> model config: $CONFIG_NAME"
    echo ">>>>>> model path: $MODEL_PATH"
    echo ">>>>>> FT data: $FT_DATA/$ft_split"
    echo ">>> Fine-tuning on $ft_split split of $EVALUATE_DATA"
    SFT_MODEL_PATH="$MODEL_PATH-$EVALUATE_DATA/mcq_ft_${ft_split}_ep${EPOCHS}_lr${LEARNING_RATE}"
    run_train "$MODEL_PATH" "$FT_DATA/$ft_split" "$SFT_MODEL_PATH" $LEARNING_RATE
    echo ">>> evaluating "
