...
INPUT_MODEL=$PROJECT_BASE_PATH/output/gpt2/random/rnd

/bin/bash "$SCRIPT_PATH"/sft_w_context_test.sh \
    "$INPUT_MODEL" \
    "$PROJECT_BASE_PATH"/output/gpt2/random/rnd-sft_qa_w_context_test \
    3

/bin/bash "$SCRIPT_PATH"/sft_w_context_train.sh \
    "$INPUT_MODEL" \
    "$PROJECT_BASE_PATH"/output/gpt2/random/rnd-sft_qa_w_context_train \
    3

/bin/bash "$SCRIPT_PATH"/sft_wo_context_test.sh \
    "$INPUT_MODEL" \
    "$PROJECT_BASE_PATH"/output/gpt2/random/rnd-sft_qa_wo_context_test \
    3
